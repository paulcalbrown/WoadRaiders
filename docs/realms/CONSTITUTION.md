...
### REALM-C-027 — Every realm is regenerated by CI, not by memory
CI MUST regenerate **every** realm in the registry and fail on any difference
from what is committed. The realms to build MUST be read from `RealmDesigns`
rather than listed, or the check goes stale the same way the artefacts do.

The guard exists because a realm can be broken by a change that never mentions
it. Setting the kit atlases to VRAM-compressed rewrote their `.import` sidecars,
which changed the resource paths the Crag's scenery resolves to — so `Crag.tscn`
stopped matching its own design. Every line of that work was in the Crypt, and
there was no reason to think about the Crag at all. Nothing failed and nothing
warned; the file simply drifted, and it was found four days later by looking at
a directory listing for an unrelated reason.

Note what the timestamps said: `Crag.json` was OLDER than `Crag.tscn`, which
reads as a stale BAKE. The truth was the opposite — the geometry was correct and
the scene was stale, because materials and textures never reach the collision
soup. A mtime is not evidence about a pipeline; regenerating is.

This gate is only meaningful if every machine agrees byte for byte, so the
artefacts' line endings are pinned in `.gitattributes` rather than left to
whatever `core.autocrlf` a contributor happens to have.
*Authority:* [.github/workflows/realms.yml](../../.github/workflows/realms.yml), [.gitattributes](../../.gitattributes)
`[checked: the workflow itself — it regenerates every realm and diffs]`
