...
//
// It joins as a MAGE, walks toward the nearest enemy, and holds the trigger.
// It then asserts, from the authoritative snapshot stream alone:
...
{
    Console.WriteLine("[probe] connected — joining as a Mage");
    peer.Send(NetProtocol.Frame(MessageType.JoinRequest,
        new JoinRequest { Name = "ClassProbe", Class = (byte)CharacterClass.Mage }), 0, DeliveryMethod.ReliableOrdered);
};
...
var pass = welcomed && sawMyClassAsMage && healthOk && sawMagicBolt && sawEnemyHurt;
