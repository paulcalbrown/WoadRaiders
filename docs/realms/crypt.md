...
| D7 | `SPACE-007` (east flight) | The span's south kerb row breaks over the east flight's mouth (x ≥ 6880) | The parapet ran across the stair's top: the join never meshed, and "tops out ON the deck" shipped sealed behind kerbstones with sub-body gaps. The old physics dropped climbers through the tread gap there (`TryLedgeDrop`); navmesh-only movement handed them to the lip's plunge links instead — read in play as falling through the stairs. A stair's mouth must be OPEN, not merely overlapped; no automated check sees "the flight doesn't climb", since a one-way descent strands nobody. | 2026-07-26 |
